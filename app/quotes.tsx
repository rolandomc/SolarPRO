      ...
      const cargarCotizacion = async () => {
        try {
          ...
          if (data) {
            ...
            if (cotizacion) {
              setCotizacionId(cotizacion.id); setCliente(cotizacion.cliente); setItems(cotizacion.items || []);
            }
          }
        } catch (error) {}
      };
    ...
    <View style={{ flex: 1 }}>
      <View style={[styles.container, dynamicStyles.container]}>
        ...
        <View style={styles.headerRow}>
          ...
          {cotizacionId && (
            <TouchableOpacity onPress={limpiarFormulario} style={styles.cancelButton}><Text style={{color: '#FFF', fontWeight: 'bold'}}>Cancelar</Text></TouchableOpacity>
          )}
        </View>
        ...
        <View style={[styles.card, dynamicStyles.card, {padding: 16, marginBottom: 16, borderRadius: 12, borderWidth: 1}]}>
          <TextInput style={[styles.input, dynamicStyles.input, {marginBottom: 10}]} placeholder="Descripción del artículo" placeholderTextColor={isDark ? "#64748B" : "#94A3B8"} value={desc} onChangeText={setDesc} />
          <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <TextInput style={[styles.input, dynamicStyles.input, {width: '48%'}]} placeholder="Cant." keyboardType="numeric" placeholderTextColor={isDark ? "#64748B" : "#94A3B8"} value={cant} onChangeText={setCant} />
            <TextInput style={[styles.input, dynamicStyles.input, {width: '48%'}]} placeholder="Precio" keyboardType="numeric" placeholderTextColor={isDark ? "#64748B" : "#94A3B8"} value={prec} onChangeText={setPrec} />
          </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[styles.itemRow, dynamicStyles.card]}>
              ...
              <TouchableOpacity onPress={() => eliminarItem(item.id)}><Ionicons name="trash" size={20} color="#EF4444" /></TouchableOpacity>
            </View>
          )}
        />
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity style={styles.pdfButton} onPress={procesarCotizacion}>
            ...
            <Text style={{color: "#FFF", fontWeight: "bold", marginLeft: 8}}>{cotizacionId ? "Actualizar" : "Generar PDF"}</Text>
          </TouchableOpacity>
        </View>
      </View>

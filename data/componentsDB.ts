...
// Precios en MXN (referencia 2025, varian por distribuidor ~±15%)
...
export interface Inversor {
  ...
  pmax_ac: number;        // W salida AC
  max_dc_input: number;   // W entrada DC maxima total
  max_dc_volts: number;   // V maximo en entrada DC (limite string)
  v_ac: number; fases: number; i_max_ac: number;
  num_mppt: number;           // cantidad de trackers MPPT
  imax_por_mppt: number;      // A maximos por MPPT (corriente string)
  precio_mxn: number;
}
...
export const INVERSORES_DB: Inversor[] = [
  // Growatt MIN — Residencial monofasico
  ...
  // Growatt SPH — Hibrido con bateria
